...
const setup = async function() {
  ...
  // Restore the default configuration file if necessary.
  restore();
...
const restore = async function() {
  ...
  // Get paths.
  const _backup = `${_config}.backup`;
  const _default = `${_config}.default`;
  
  // Check if the configuration file is a symbolic link.
  if( fs.lstatSync(_config).isSymbolicLink() ) {
    
    // Remove the link.
    fs.removeSync(_config);
    
    // Attempt to restore the configuration file from the backup file.
    if( fs.existsSync(_backup) && !fs.lstatSync(_backup).isSymbolicLink() ) fs.copySync(_backup, _config);
    
    // Otherwise, restore the configuration file from the default.
    else fs.copySync(_default, _config);
