...
class Mync {
  ...
  constructor() {
    ...
    this.MYNCRC = yaml.parse(fs.readFileSync(path.resolve(CWD, '.myncrc'), 'utf8'));
    this.STORAGE = () => {
      
      // Get the storage volume.
      const volume = _.startsWith(this.MYNCRC.STORAGE_VOLUME, '~/') ? untildify(this.MYNCRC.STORAGE_VOLUME) : this.MYNCRC.STORAGE_VOLUME;

      // Return the storage location.
      return path.resolve(`${volume}/${this.MYNCRC.STORAGE_DIRECTORY}`);
      
    };
    this.BACKUP = () => path.join(this.STORAGE(), `.backup/${this.ID}`);
    ...
    this.METADATA = store({
      path: path.join(CWD, '/.metadata.json'),
      indent: 2
    });
    this.CONFIG = yaml.parse(fs.readFileSync(path.resolve(this.STORAGE(), this.MYNCRC.USER_CONFIG), 'utf8'));
  ...
  set storage( volume ) {
    ...
    if( fs.existsSync(path.resolve(volume)) ) {
      ...
      // Generate a new MYNCRC file using the the template and the updated data.
