...
module.exports = function push( options ) {
  ...
  const write     = async function( type, src, dest, defer = null ) {
    ...
    // Otherwise, move on.
      ...
      logger.warning(`Skipped ${src}. ${earl.say(error.code)}`);
  ...
  const cancel    = function() {
    ...
    logger.error('Push operation canceled. All unchanged settings on your workstation were left intact.');
  ...
  processr(settings, push).then(() => success()).catch((error) => fail(error));
