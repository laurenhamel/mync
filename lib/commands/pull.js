...
module.exports = function pull( options ) {
  ...
  const overwrite = async function( type, src, dest, defer = null ) {
    ...
    try {
      ...
      if( /^y(es)?$/.test(overwrite) ) await write(type, src, dest, defer);
      ...
      switch( error.message ) {
        ...
        default: logger.warning(`Skipped ${dest}. ${earl.say(error.code, type)}`);
  ...
  const fail      = function( error ) {
    ...
    switch( error.message ) {
      ...
      default: logger.error('Pull failed. An error occurred. Please check your configurations and try again.');
  ...
  processr(_settings, pull).then(() => success()).catch((error) => fail(error));
